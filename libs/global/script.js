var config = {
    ...
    goToDunHuang: function() {
        goTo(40.14211,94.66159);
        this.autoSpin = false;
        showLoction();
    },
...
function init(ref) {
    ...
    gui.add(config, 'goToDunHuang');
    gui.add(config,'Return')
    // gui.add(config, 'zoom', 0, 1).listen();
    // gui.domElement.style.z = 301;
...
function TurnTo() {
    ...
    isTweening = true;
    TweenMax.to(config, 1, {delay: 0, zoom: 0, ease:'easeInOutSine', onComplete: function(){
        isTweening = false;
    }});
}
